...
class ListConstraint(object):
    ...
    def verify(self, val):
        return (
            isinstance(val, list)
            and self.verify_items(val)
            and self.min_len <= len(val) <= self.max_len
        )
...
def verify_types(io_pairs, sig):
    ...
    for pair in io_pairs:
        ...
        try:
            ...
            print(
                "ERROR: unable to verify input type constraints ({}) for value ({})".format(
                    in_type, i
                )
            )
            raise e
        try:
            ...
            print(
                "ERROR: unable to verify output type constraints ({}) for value ({})".format(
                    out_type, o
                )
            )
            raise e
